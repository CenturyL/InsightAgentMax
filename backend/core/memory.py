...
_INVALID_FACT_VALUES = {"什么", "什么名字", "谁", "未知", "未提供", "没有提供", "不清楚"}


class LongTermMemoryConfigurationError(RuntimeError):
    pass


def ensure_pgvector_ready() -> str:
    if not settings.POSTGRES_URL:
        raise LongTermMemoryConfigurationError("POSTGRES_URL 未配置，无法启用长期记忆。")
    try:
            ...
            with conn.cursor() as cur:
                ...
                if not available:
                    raise LongTermMemoryConfigurationError(
                        "PostgreSQL 未安装 pgvector 扩展文件，请使用带 pgvector 的 PostgreSQL 镜像。"
                    )
                cur.execute("CREATE EXTENSION IF NOT EXISTS vector")
                cur.execute("SELECT extversion FROM pg_extension WHERE extname = 'vector'")
                installed = cur.fetchone()
                if not installed:
                    raise LongTermMemoryConfigurationError("pgvector 扩展创建后仍不可用。")
                return str(installed[0])
    except LongTermMemoryConfigurationError:
        raise
    except Exception as exc:
        raise LongTermMemoryConfigurationError(f"pgvector 就绪检查失败：{type(exc).__name__}") from exc
...
def normalize_memory_fact(content: str) -> str | None:
    ...
    if not value or value in _INVALID_FACT_VALUES:
        return None
    return text


def memory_document_id(user_id: str, normalized_fact: str) -> str:
    return str(uuid.uuid5(uuid.NAMESPACE_URL, f"long-term-memory:{user_id}:{normalized_fact}"))
...
class LongTermMemoryManager:
    ...
    def store(self) -> PGVector:
        ...
        if self._store is None:
            if not settings.LONG_TERM_MEMORY_ENABLED:
                raise LongTermMemoryConfigurationError("长期记忆已通过配置关闭。")
            if not settings.POSTGRES_URL:
                raise RuntimeError(
    ...
    def save(self, user_id: str, content: str) -> None:
        ...
        self.store.add_documents([doc], ids=[memory_document_id(user_id, normalized)])
