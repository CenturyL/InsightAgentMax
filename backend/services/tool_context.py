...
_model_choice_var: ContextVar[str] = ContextVar("tool_model_choice", default="deepseek-v4-flash")
...
# loop_guard 读的工具调用历史（按请求隔离），每条记录 {tool, args_hash, args_preview}
_tool_call_history_var: ContextVar[list[dict[str, Any]]] = ContextVar("tool_call_history", default=[])
# loop_guard 已经给当前请求注入过收敛指令（避免重复注入）
# 用 list[str] 包装，跨 asyncio 任务时就地修改 [0] 而非 .set() 重绑
_guard_fired_var: ContextVar[list[str]] = ContextVar("guard_fired", default=[""])
...
def set_tool_request_context(
    *,
    ...
    return {
        ...
        "tool_call_history": _tool_call_history_var.set([]),
        "guard_fired": _guard_fired_var.set([""]),
    }
...
def reset_tool_request_context(tokens: dict[str, Any]) -> None:
    ...
    _tool_call_history_var.reset(tokens["tool_call_history"])
    _guard_fired_var.reset(tokens["guard_fired"])
...
def record_tool_call(tool_name: str, args: Any) -> dict[str, Any]:
    """把一次工具调用写入 per-request 历史 ring buffer，供 loop_guard 读。

    注意：LangGraph 里 tool 调用与下一次 model 调用常处于不同 asyncio 任务，
    每个子任务拿到的是 Context 的副本；因此绝不能 `.set(new_list)` 重新绑定
    （那只会改写子任务自己的副本），必须就地 mutate 同一个 list 对象，
    这样父任务里的 loop_guard 才能看到新记录。
    """
    import hashlib
    import json as _json
    try:
        args_text = _json.dumps(args, ensure_ascii=False, sort_keys=True, default=str)
    except Exception:
        args_text = str(args)
    args_hash = hashlib.md5(args_text.encode("utf-8")).hexdigest()[:12]
    entry = {
        "tool": tool_name,
        "args_hash": args_hash,
        "args_preview": args_text[:160],
    }
    history = _tool_call_history_var.get()
...
def get_tool_call_history() -> list[dict[str, Any]]:
    return list(_tool_call_history_var.get())


def get_guard_fired() -> str:
    return _guard_fired_var.get()[0]


def set_guard_fired(phase: str) -> None:
    _guard_fired_var.get()[0] = phase
