...
from backend.agents.state import OrchestratorState, StepResult
from backend.core.llm import get_model_by_choice
from backend.runtime.budget import consume_pae_step_if_active, invoke_model
...
async def _run_analysis_step(state: OrchestratorState, step_query: str, step_goal: str) -> str:
    model = get_model_by_choice(state.get("model_choice", "deepseek-v4-flash"))
    prompt = (
        "你正在执行计划中的一个分析步骤。\n"
    ...
    response = await invoke_model(model, prompt)
...
async def execute_plan_events(
    ...
    context_tokens = set_tool_request_context(
        ...
        model_choice=state.get("model_choice", "deepseek-v4-flash"),
    ...
    try:
        ...
        for step in plan:
            await consume_pae_step_if_active()
