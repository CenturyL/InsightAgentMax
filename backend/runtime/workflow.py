...
from backend.runtime.budget import consume_pae_if_active, invoke_model


def _resolve_pae_model_choice(model_choice: str) -> str:
    """
    PAE 默认使用 Flash；用户明确选择 Pro 时保持原选择。

    本地聊天模型已下线，不再存在 PAE 静默切回 Ollama 的分支。
    """
    return model_choice or "deepseek-v4-flash"
